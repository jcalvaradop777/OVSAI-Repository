...
export default function EditarEstacion({ id }) {
      ...
      for (let dato in datos) {
        setData((prev) => {
          return {
            ...prev,
            [dato]:
              datos[dato] != null || datos[dato] != undefined
                ? datos[dato]
                : "",
          };
        });
  ...
  const obtenerDataEstacion = async (_id) => {
    await fetch(`/api/estaciones/getOne/${_id}`)
      .then(async (res) => await res.json())
      .then((datos) => {
        // Promesa: una vez existe una respuesta a la promesa, en THEN capturo dicha respuesta asincrona en variable datos
        if (datos != null || datos != undefined) {
          procesarDatos(datos[0]);
          setLoad(true);
        } else {
          setLoad(true);
        }
    ...
    <form onSubmit={actualizar} className="w-full">
      ...
      <label htmlFor="geologial">
        ...
        >
          ...
          <option value="EDIFICIO O INFRAESTRUCTURA">
